    ...
    const fetchUserType = async () => {
      ...
      if (token) {
        try {
          ...
          setUserType(decoded.userType); // Set userType from the decoded token
        } catch (error) {
    ...
    <header className={`navbar ${isDarkMode ? 'dark-mode' : ''}`}>
      ...
      <nav className={`navbar-menu ${isMobileMenuOpen ? 'open' : ''}`}>
        <ul className="navbar-links">
          ...
          <li><Link to="/forum"><FontAwesomeIcon icon={faComments} /> Forums</Link></li>
      ...
      <div className="navbar-profile" ref={profileRef}>
        ...
        {isDropdownOpen && (
          <ul className="profile-menu" role="menu">
            <li role="none">
              <Link to={userType === 'doctor' ? '/clinician-profile' : '/individual-profile'} role="menuitem">
                Edit Profile
              </Link>
            </li>
