...
function RebookAppointment() {
  ...
  const confirmRebook = async () => {
    try {
      ...
      setMessage('An error occurred while rescheduling the appointment.');
    }
  };
  ...
  return (
    <div className="rebook-container">
      <form onSubmit={handleSubmit} className="rebook-form">
        ...
        <div className="form-group">
          <label htmlFor="new_reason">Reason for Rescheduling:</label>
          <select
            id="new_reason"
            name="new_reason"
          ...
          >
            ...
            <option value="Doctor Availability">Availability</option>
        ...
        <div className="modal-overlay" onClick={handleModalClose}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h5 className="modal-title">Confirm Rescheduling</h5>
              <button type="button" className="btn-close" onClick={handleModalClose} aria-label="Close"></button>
            </div>
            <div className="modal-body">
              Are you sure you want to reschedule this appointment with the new details?
            </div>
            <div className="modal-footer">
              <button type="button" className="btn btn-secondary" onClick={handleModalClose}>Cancel</button>
              <button type="button" className="btn btn-primary" onClick={confirmRebook}>Reschedule Appointment</button>
            </div>
          </div>
        </div>
      )}
